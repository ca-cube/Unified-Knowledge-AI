        ...
        <div className="flex h-screen bg-[#0a0a0c] text-slate-200 overflow-hidden">
            ...
            <aside className="w-64 border-r border-white/10 bg-black/40 backdrop-blur-md flex flex-col">
                ...
                <nav className="flex-1 p-4 space-y-2">
                    ...
                    <SidebarItem icon={Database} label="Knowledge Sources" />
                    <SidebarItem icon={Scale} label="Policy Engine" />
                    <SidebarItem icon={History} label="Audit Trace" />
                    <SidebarItem icon={Network} label="Compliance Graph" />
                </nav>

                <div className="p-4 border-t border-white/10 mt-auto">
                    <div className="p-3 glass-card text-xs flex items-center gap-2">
                        <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                        <span>GDPR/HIPAA Compliant Session</span>
                    </div>
                </div>
            </aside>
                            ...
                            {showResult && resultData && (
                                ...
                                >
                                    ...
                                    <div className="col-span-1 space-y-6">
                                        ...
                                        {resultData.governanceAlerts.length > 0 && (
                                            <div className="glass-card p-6 bg-amber-500/5 border-amber-500/20">
                                                <h3 className="text-xs font-bold uppercase text-amber-500 mb-4 flex items-center gap-2">
                                                    <AlertCircle className="w-4 h-4" />
                                                    Governance Alerts
                                                </h3>
                                                {resultData.governanceAlerts.map((alert: string, idx: number) => (
                                                    <p key={idx} className="text-[11px] text-amber-200/60 leading-tight">
                                                        {alert}
                                                    </p>
                                                ))}
                                            </div>
...
function SidebarItem({ icon: Icon, label, active = false }: { icon: any, label: string, active?: boolean }) {
    return (
        <div className={`flex items-center gap-3 px-3 py-2 rounded-lg cursor-pointer transition-all ${active ? 'bg-blue-600/10 text-blue-400 border border-blue-500/20' : 'text-slate-400 hover:bg-white/5 hover:text-white'
            }`}>
